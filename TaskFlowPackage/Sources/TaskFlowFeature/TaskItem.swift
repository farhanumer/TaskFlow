...
public struct TaskItem: Identifiable, Equatable, Sendable {
    ...
    public init(id: UUID = UUID(), title: String, isDone: Bool = false, isFavorite: Bool = false, category: TaskCategory = .personal) {
        self.id = id
        self.title = title
        self.isDone = isDone
...
extension TaskItem {
    public static let samples: [TaskItem] = [
        ...
        TaskItem(title: "Wire up Linear issue workflow", isFavorite: true, category: .work),
        TaskItem(title: "Record walkthrough", category: .work),
        TaskItem(title: "Morning run", isDone: true, isFavorite: true, category: .health),
        ...
        TaskItem(title: "Plan weekend trip", category: .personal),
